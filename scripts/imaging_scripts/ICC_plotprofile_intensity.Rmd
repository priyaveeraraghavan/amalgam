---
title: "Proximal/Distal Immunofluoresence in CPN Axons In Vitro"
...
get_dapi_profiles <- function(rbp_meta) {
  ...
  plot(ggplot(dapi_profiles, 
         ...
         facet_wrap(vars(is_soma)))
}

```
```{r}
get_channel_meangray_bin <- function(file, dapi_file, dapi_thresh, num_bins=40) {
  ...
  # what is the dapi signal in soma
  mod <- Mclust(log10(subset(res_dapi, is_axon == 0)$gray_value))
  # get clustering parameters
  summary_mod <- summary(mod, parameters=TRUE)
  # true DAPI will be the biggest value
  which_clust <- which.max(summary_mod$mean)
  num_clust <- length(summary_mod$mean)
  dapi_thresh <- 10^qnorm(0.01, 
                       mean=summary_mod$mean[which_clust], 
                       sd=sqrt(summary_mod$variance[which_clust]))
  
  plot(ggplot(res_dapi, aes(gray_value)) + 
    geom_density() + geom_vline(xintercept=dapi_thresh) +
      scale_x_log10() +
      facet_wrap(vars(is_axon)))
...
```
```{r}
cpeb2_meta <- get_meta_from_files("Cpeb2")
cpeb2_meta$dapi_thresh <- 5000
get_dapi_profiles(cpeb2_meta)


cpeb2_neuron_bins <- suppressMessages(calculate_all_neurons(cpeb2_meta))

plot_soma(cpeb2_neuron_bins, file.path(plot_dir, "Cpeb2_soma_intensities.pdf"))

cpeb2_neuron_bin_stats <- calculate_stats_all_neurons(cpeb2_neuron_bins)
plot_axon(cpeb2_neuron_bin_stats, file.path(plot_dir, "Cpeb2_axon_profiles.pdf")) 
