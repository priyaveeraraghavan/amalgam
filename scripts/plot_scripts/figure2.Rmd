...
plot_main_2a <- function(plot_savefile) {
  ...
  plt <- rasterize(ggplot(res_gc_toplot, 
    ...
    rasterize(geom_point(data=subset(res_gc_toplot, is_cpn_subtype_gc & is_cpn_over_input & !is_forebrain_gcf),
               mapping=aes(log2FoldChange, -log10(padj)),
               color=cpn_color,
               size=0.5), dpi=300) +
    rasterize(geom_point(data=subset(res_gc_toplot, is_cth_subtype_gc & is_cth_over_input & !is_thalamus_gcf),
               mapping=aes(log2FoldChange, -log10(padj)),
               color=cthpn_color,
               size=0.5), dpi=300) +
  ...
  plot(plt)
   ...
   num_cpn_specific <- res_gc_toplot %>% subset(is_cpn_subtype_gc & is_cpn_over_input & !is_forebrain_gcf) %>% nrow()
   num_cth_specific <- res_gc_toplot %>% subset(is_cth_subtype_gc & is_cth_over_input & !is_thalamus_gcf) %>% nrow()
   num_shared <- res_gc_toplot %>% subset(!is_cpn_subtype_gc & !is_cth_subtype_gc) %>% nrow()
   
   print(sprintf("Number CPN specific: %s\nNumber CThPN specific: %s\nNumber Shared: %s",
                 num_cpn_specific, num_cth_specific, num_shared))
}
...
plot_main_2b <- function(plot_savefile) {
  ...
  res_gc_toplot <- res_gc %>% 
    subset((is_gcf_constant & (is_cpn_over_input | is_cth_over_input)) |
             ...
             (is_cth_subtype_gc & is_cth_over_input & is_forebrain_gcf)) %>%
    rowwise() %>%
    mutate(gc_subtype=ifelse(is_cpn_subtype_gc & is_cpn_over_input & !is_forebrain_gcf, "CPN", 
                             ifelse(is_cth_subtype_gc & is_cth_over_input & !is_thalamus_gcf, "CTh", "Shared")))
