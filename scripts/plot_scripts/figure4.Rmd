...
plot_main_4b <- function(plot_savefile) {
  load(file.path(file_path_ref$project_directory, file_path_ref$sequencing$results$gc_vs_soma_utr_motifs)) #dpc23_cpn_cth_gc_vs_soma_utr_motifs))
  ...
  both_enrichment <- cpn_enrichment %>% 
    ...
    mutate(enrichment_type=ifelse(padj.cpn < 0.05 & mean_enrichment.cpn > 1,
                                  ...
                                  "Neither Enriched")) %>%
    rowwise() %>%
    mutate(motif_rbps=gsub("BRUNOL", "CELF", ifelse(motif_rbps=="A2BP1", "RBFOX1", motif_rbps)))
  
  both_enrichment_labels <-  subset(both_enrichment, motif_rbps %in% c("RBFOX1", "ANKRD17", "CELF2", "CELF4", "CELF6", "CPEB4", "ELAVL2", "ELAVL3", "FMR1", "G3BP2", "HNRNPF", "HNRNPH2", "HNRNPR", "KHDRBS2", "NOVA2", "PABPC1", "PTBP2", "SYNCRIP", "hnRNPK"))
    
   # subset(both_enrichment, motif_rbps %in% c("RALY", "CPEB4", "TIA1", "SFPQ", "QKI", "FMRP", "FMR1", "FXR1", "FXR2", "IGF2BP1", "IGF2BP2", "IGF2BP3", "PUM1", "PUM2"))
...
```
Run STREME to find de novo motifs
```{}
#!/bin/bash
...
#SBATCH -t 0-20:00:00
#SBATCH --mem=20G
#SBATCH -o run_meme_dpc23_cpn.out
#SBATCH -e run_meme_dpc23_cpn.err

set +eu
source activate rnaseq_env_current

gc_fasta=CPN_GC_3utr.fa
bkg_fasta=CPN_Soma_3utr.fa
outdir=20230415_streme_CPN_GC_DPC23
streme --oc $outdir --objfun de --n $bkg_fasta --rna --align right --p $gc_fasta
